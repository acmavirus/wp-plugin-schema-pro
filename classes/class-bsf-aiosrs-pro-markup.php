...
if ( ! class_exists( 'BSF_AIOSRS_Pro_Markup' ) ) {
	...
	class BSF_AIOSRS_Pro_Markup {
		...
		public function init() {
			...
			add_filter( 'the_content', array( $this, 'rating_markup' ), 9 );
			add_action( 'wp_head', array( $this, 'generate_ratings_css' ) );
		...
		/**
		 * Generate Dynamic CSS for Star Ratings
		 *
		 * @return void
		 */
		public function generate_ratings_css() {
			$settings = BSF_AIOSRS_Pro_Admin::get_options( 'wp-schema-pro-ratings-settings' );
			if ( '1' !== $settings['enable-star-ratings'] ) {
				return;
			}

			$color = ! empty( $settings['star-color'] ) ? $settings['star-color'] : '#ffcc00';
			$size  = ! empty( $settings['star-size'] ) ? $settings['star-size'] : '20';

			echo '<style id="wp-schema-pro-ratings-dynamic-css">';
			echo '.aiosrs-star-rating-wrap .aiosrs-star-rating { color: ' . esc_attr( $color ) . '; font-size: ' . esc_attr( $size ) . 'px; width: ' . esc_attr( $size ) . 'px; height: ' . esc_attr( $size ) . 'px; line-height: ' . esc_attr( $size ) . 'px; }';
			echo '.aiosrs-star-rating-wrap .aiosrs-star-rating:before { font-size: ' . esc_attr( $size ) . 'px; }';
			echo '.aiosrs-rating-wrap { margin: 15px 0; display: flex; align-items: center; gap: 10px; }';
			echo '.aiosrs-rating-text { font-weight: bold; }';
			echo '.aiosrs-star-rating.dashicons-star-empty { color: #ccc; }';
			echo '</style>';
		}
		...
		public function aiosrs_user_rating_callback() {
			if ( ! wp_verify_nonce( $_POST['nonce'], 'schema-pro-user-rating' ) ) {
		...
		public function rating_markup( $content = '' ) {
			...
			// Global Ratings Logic
			if ( '1' === $settings['enable-star-ratings'] && is_array( $settings['post-types'] ) && in_array( $post_type, $settings['post-types'] ) ) {
				$position = $settings['ratings-position'];
				$markup   = $this->get_global_rating_html( $post_id, $settings );
			...
			// Original Schema-specific Ratings Logic
		...
		/**
		 * Get Global Rating HTML
		 *
		 * @param int   $post_id Post ID.
		 * @param array $settings Ratings settings.
		 * @return string
		 */
		public function get_global_rating_html( $post_id, $settings ) {
			// Check if we already have a dedicated schema for this post
			// If so, we use its ID to store ratings so it counts towards the schema
			$schema_posts = self::get_schema_posts();
			$schema_id    = 0;
			if ( ! empty( $schema_posts ) ) {
				reset( $schema_posts );
				$schema_id = key( $schema_posts );
			}
			...
			ob_start();
			?>
			<div class="aiosrs-rating-wrap global-rating" data-schema-id="<?php echo esc_attr( $schema_id ); ?>">
				<?php if ( ! empty( $settings['rating-text'] ) ) : ?>
					<span class="aiosrs-rating-text"><?php echo esc_html( $settings['rating-text'] ); ?></span>
				<?php endif; ?>
				
				<?php BSF_AIOSRS_Pro_Custom_Fields_Markup::get_star_rating_markup( $avg_rating ); ?>
			...
			if ( is_array( $result ) && ! empty( $result ) ) {
				...
				foreach ( $result as $post_id => $post_data ) {
					...
					if ( true == $enabled ) {
						...
						echo wp_json_encode( BSF_AIOSRS_Pro_Schema_Template::get_schema( $current_post_id, $post_id, $schema_type, $schema_meta ), JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT );
						echo '</script>';
						echo '<!-- / Schema optimized by Schema Pro -->';
					}
		...
		static function global_schema_markup( $post_id, $type ) {
			...
			if ( true == $enabled_global_schema ) {
				...
				echo '<script type="application/ld+json">';
				echo wp_json_encode( BSF_AIOSRS_Pro_Schema_Template::get_global_schema( $post_id, $type ), JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT );
				echo '</script>';
				echo '<!-- / ' . $type . ' Schema optimized by Schema Pro -->';
			}
