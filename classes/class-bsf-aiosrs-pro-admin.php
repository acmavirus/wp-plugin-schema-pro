...
if ( ! class_exists( 'BSF_AIOSRS_Pro_Admin' ) ) {
	...
	class BSF_AIOSRS_Pro_Admin {
		...
		public function register_settings() {
			...
			register_setting( 'wp-schema-pro-ratings-settings-group', 'wp-schema-pro-ratings-settings' );
		...
		public static function get_options( $setting = 'aiosrs-pro-settings' ) {
			...
			if ( ! isset( self::$setting_option[ $setting ] ) || empty( self::$setting_option[ $setting ] ) ) {
				...
				$defaults = array(
					...
					'wp-schema-pro-ratings-settings' => array(
						'enable-star-ratings' => 'disabled',
						'ratings-position'    => 'bottom',
						'star-color'          => '#ffcc00',
						'star-size'           => '20',
						'rating-text'         => __( 'Đánh giá:', 'wp-schema-pro' ),
						'post-types'          => array( 'post', 'product' ),
					),
