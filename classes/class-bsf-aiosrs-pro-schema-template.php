	...
	class BSF_AIOSRS_Pro_Schema_Template {
		...
		public static function get_meta_value( $schema_id = 0, $post = 0, $meta_data = array(), $key = '', $type = 'text', $create_field = '', $single = true ) {
			...
			if ( 'schema-type' == $key || 'reviewer-type' == $key ) {
				...
				// KK Star Ratings Integration
				$kksr_casts = get_post_meta( $post['ID'], '_kksr_casts', true );
				if ( empty( $kksr_casts ) ) {
					$kksr_casts = get_post_meta( $post['ID'], '_kk_star_ratings_counts', true );
				}
				if ( empty( $kksr_casts ) ) {
					$kksr_casts = get_post_meta( $post['ID'], '_kk_star_ratings_casts', true );
				}
				
				if ( ! empty( $kksr_casts ) ) {
					$review_counts = $kksr_casts;
				}

				// Try global ratings if count is zero or none
				if ( ( empty( $review_counts ) || 'none' == $review_counts ) && 'accept-user-rating' != $meta_data['rating'] ) {
					$rating_settings = BSF_AIOSRS_Pro_Admin::get_options( 'wp-schema-pro-ratings-settings' );
					if ( '1' == $rating_settings['enable-star-ratings'] && in_array( get_post_type( $post['ID'] ), $rating_settings['post-types'] ) ) {
						$review_counts = get_post_meta( $post['ID'], 'bsf-schema-pro-review-counts-0', $single );
					}
				}

				return apply_filters( 'wp_schema_pro_review_count', $review_counts, $post['ID'], $schema_id );
			}
			...
			if ( empty( $post ) || empty( $schema_key ) || 'none' == $schema_key ) {
				...
				switch ( $schema_key ) {
					...
					case 'accept-user-rating':
						...
						// KK Star Ratings Integration
						$kksr_ratings = get_post_meta( $post['ID'], '_kksr_ratings', true );
						$kksr_casts   = get_post_meta( $post['ID'], '_kksr_casts', true );
						$kksr_avg     = get_post_meta( $post['ID'], '_kksr_avg', true );

						if ( empty( $kksr_ratings ) && empty( $kksr_casts ) ) {
							$kksr_ratings = get_post_meta( $post['ID'], '_kk_star_ratings', true );
							$kksr_casts   = get_post_meta( $post['ID'], '_kk_star_ratings_counts', true );
						}

						if ( ! empty( $kksr_avg ) ) {
							$value = $kksr_avg;
						} elseif ( ! empty( $kksr_casts ) && ! empty( $kksr_ratings ) ) {
							$value = round( $kksr_ratings / $kksr_casts, 1 );
							if ( $value > 5 ) {
								$value = $kksr_ratings; // Fallback if ratings was already average
							}
						}

						// Try global ratings if value is zero or none
						if ( ( empty( $value ) || 0 == $value ) && 'accept-user-rating' != $meta_data['rating'] ) {
							$rating_settings = BSF_AIOSRS_Pro_Admin::get_options( 'wp-schema-pro-ratings-settings' );
							if ( '1' == $rating_settings['enable-star-ratings'] && in_array( get_post_type( $post['ID'] ), $rating_settings['post-types'] ) ) {
								$value = get_post_meta( $post['ID'], 'bsf-schema-pro-rating-0', $single );
							}
						}

						$value = apply_filters( 'wp_schema_pro_rating_value', $value, $post['ID'], $schema_id );
						break;
